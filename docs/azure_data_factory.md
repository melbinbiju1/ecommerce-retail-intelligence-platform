...
```text
I added Azure Data Factory as the cloud orchestration layer. I created linked services for Azure Blob Storage and Azure SQL Database, built source and sink datasets, and created a copy pipeline that moves the raw Olist orders CSV from Blob Storage into an Azure SQL staging table. I also verified the pipeline output from Python using row counts and timestamp checks.
```